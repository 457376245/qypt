...
import com.web.freemarker.custom.DateTimeMethod;
import com.web.freemarker.custom.FormatLocalDateTime;
import com.web.freemarker.directive.DirectiveUtils;
 ...
 * 1.datatime(输出字符串格式)
 * 宏
 * 1.extends 模板继承 <@extends name="/template.html"/>
 * 2.block 定义模模块 <@block name="header"></@block>
 * 3.override 模块重写父的内容 <@override name="header"> </@override>
 * 4.super 模块继承父的内容 <@override name="header"><@super/> </@override>
 *<P>
...
public class FreemarkerHandler 
{
	...
	public Properties getFreemarkerSettings(){
		...
		InputStream in=null;
		try {
			in=FreemarkerHandler.class.getResourceAsStream("/com/web/parameter/parameter.properties");
			properties.load(in);
			return properties;
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void createConfigureAll() throws  TemplateException{
		...
		tdmMap.put("formatLocalDateTime",new FormatLocalDateTime());
