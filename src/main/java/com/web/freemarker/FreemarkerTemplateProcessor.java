...
import com.web.filter.CsrfPreventionFilter;
...
public class FreemarkerTemplateProcessor implements InitializingBean{
	static Logger log = LoggerFactory.getLogger(CsrfPreventionFilter.class);
	...
	public void afterPropertiesSet() throws Exception {
		...
		if(exposeRapidMacros) {
			log.debug("expose rapid macros: <@block> <@extends> <@override> for freemarker");
	...
	public static void processTemplate(Template template,Object model,Writer out) throws FreemarkerTemplateException {
		try {
			...
			throw new FreemarkerTemplateException("process template occer " +
					"IOException,templateName:"+template.getName(),e);
		} catch (TemplateException e) {
			throw new FreemarkerTemplateException("process template occer " +
					"TemplateException,templateName:"+template.getName(),e);
		}
	}
